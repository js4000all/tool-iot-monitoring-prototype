...
### 10.2 タスク台帳と完了記録の置き場所

GitHub の PR 本文や issue tracker はレビューには便利ですが、AI エージェントが常に過去 PR データへ自律的にアクセスできるとは限りません。そのため、後続タスクが必要とする完了情報と設計判断はリポジトリ内にも残します。

- 進捗の正本は issue tracker または project board に置いてよい。
- リポジトリ内では [progress/README.md](progress/README.md) を索引として、完了したタスクごとの記録を `docs/progress/` 配下に残す。
- 各タスクの完了記録は [templates/task-completion.md](templates/task-completion.md) を使う。
- 完了記録には「ここまで終えたこと」「完成したもの」「なぜそうしたか」「後続タスクへの前提」を書く。
- 大きな台帳を毎 PR で編集してコンフリクトを起こさないよう、各タスクは原則として個別ファイルに記録する。
- Phase 全体の完了や計画変更が必要な場合だけ、このドキュメントへ反映する。
...
## 11. タスク間のコンテキストと設計判断の継承

ここでの引き継ぎは、作業途中で人に託すためのものではなく、各タスクまたは PR の完了後に、後続タスクが前提を誤らないようにするためのものです。

### 11.1 PR とリポジトリの両方に残す情報

各 PR は [templates/pr-description.md](templates/pr-description.md) を使い、レビューに必要な情報を残します。さらに、後続タスクが GitHub PR データを参照できない場合に備えて、完了後に [templates/task-completion.md](templates/task-completion.md) 形式の記録を `docs/progress/` に残します。少なくとも以下を両方で追跡します。
...
- タスク完了時点の実装判断であれば `docs/progress/` のタスク完了記録
- レビュー向けの要約であれば PR 本文の `Design` と `Task Completion Record` 欄

### 11.3 タスク完了記録テンプレート

各タスクまたは PR の完了時には [templates/task-completion.md](templates/task-completion.md) を使用します。特に以下を明記します。
