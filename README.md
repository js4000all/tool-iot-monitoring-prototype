...
これらは、開発環境の制約から必ずしも実行できるわけではないと思うので、段階を分けます。
...
Level 2: 環境にMQTTブローカーを起動する手段がある場合
- MQTTを含むE2E
- ブローカー停止・復旧試験

Level 3: 実機またはARM実行環境がある場合
- Raspberry Pi上で起動
- systemd常駐確認
- 長時間運転

実行できなかった受け入れ条件は、未実施理由と必要な環境を明記すること。
実施済みのように扱わないこと。

### Development

開発・テスト・コミット規約は [docs/CONTRIBUTING.md](docs/CONTRIBUTING.md) を参照してください。

AIエージェントによる作業ルールは [docs/AGENTS.md](docs/AGENTS.md) を参照してください。
